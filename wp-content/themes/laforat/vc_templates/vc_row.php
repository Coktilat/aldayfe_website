<?php
$output = $el_class = $css = $type = $heading_color = $link_color = $link_color_hover = $text_align = $text_middle = $content_full_width = $same_height = $animation = $enable_parallax = $parallax_speed = $bg_video_src_mp4 = $bg_video_src_ogv = $bg_video_src_webm = '';
...
if ($enable_parallax) {
    ...
    $stripe_classes[] = $type=='custom-bg-video'?'stripe-parallax-video':'stripe-parallax-bg';
...
if($enable_overlay=='true'){
	$background = 'background: ' . $background_overlay;	
	$stripe_classes[] = 'have-overlay';
	$overlay .= '<div class="overlay-background" style="' . $background . '"></div>';
}
//Custom BG
$bg_video = null;
...
$output .= $overlay;
$output .= $color_style;
$output .='<div class="'.esc_attr($cl_full_width).esc_attr($cl_tex_middle).'"><div class="row'.esc_attr($cl_same_height).'">';
