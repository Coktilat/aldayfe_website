...
function jws_theme_register_form_func($atts, $content = null) {
	...
	<div class="<?php echo esc_attr(implode(' ', $class)); ?>">
		<h5 class="tb-title"><?php _e('Create a new account', 'laforat'); ?></h5>
		<p><?php _e('Create your own Unicase account', 'laforat'); ?></p>
		<?php
		...
		<form name="tb-registerform" id="tb-registerform" action="" method="post" novalidate="novalidate">
			<p>
				<label for="user_email"><?php _e('Email Address','laforat') ?></label>
				<input type="email" name="user_email" id="user_email" class="input" value="<?php echo esc_attr( wp_unslash( $user_email ) ); ?>" />
			</p>
			<p class="submit">
				<input type="submit" name="tb-wp-submit" id="tb-wp-submit" value="<?php _e('SignUp','laforat'); ?>" />
			</p>
		</form>
		<div class="tb-info">
			<h5><?php _e('SignUp today and you\'ll be able to:', 'laforat'); ?></h5>
			<ul>
				<li><?php _e('Speed your way through the checkout.', 'laforat'); ?></li>
				<li><?php _e('Track your orders easily.', 'laforat'); ?></li>
				<li><?php _e('Keep a record of all your purchases.', 'laforat'); ?></li>
			</ul>
		</div>
	</div>
    <?php
