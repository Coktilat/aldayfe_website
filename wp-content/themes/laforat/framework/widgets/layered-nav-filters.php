 ...
 * @extends  WC_Widget
 */
class laforat_WC_Widget_Layered_Nav extends WC_Widget {
	...
	public function __construct() {
		...
		$this->widget_id          = 'laforat_woocommerce_layered_nav';
		$this->widget_name        = __( '@laforat WooCommerce Layered Nav', 'woocommerce' );
	...
	public function widget( $args, $instance ) {
		...
		if ( 0 < count( $terms ) ) {
			...
			// check is_color layered
			$is_color = strpos( strtolower( $instance['title'] ), 'color') !== false;
			...
			if ( 'dropdown' == $display_type ) {
				...
				$inline_style = '<style>';
				// $wrap_class = ( $is_color ) ? 'class="layered-nav-color"' : '';

				// List display
				if( !empty( $instance['sub_title'] ) ){
					echo '<small>'. esc_attr( $instance['sub_title'] ) .'</small>';
				}
				echo '<ul>';
				...
				foreach ( $terms as $term ) {
					...
					if( $is_color ){
						$class = str_replace( 'class="chosen"', ' chosen', $class );
						$attrColor = esc_attr( strtolower( $term->name ) );
						$class = 'class="color-attribute color-'. strtolower( $term->name ) . $class .'"';

						$inline_style .= '.color-'.$attrColor.' a:before{background-color:'.$attrColor.'!important;}';
					}
...
function register_laforat_layered_nav() {
    register_widget('laforat_WC_Widget_Layered_Nav');
}
add_action('widgets_init', 'register_laforat_layered_nav');
