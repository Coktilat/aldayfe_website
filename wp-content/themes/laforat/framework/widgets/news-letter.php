 ...
 */
class laforat_NewsletterWidget extends WP_Widget {

    function __construct() {
        parent::__construct(false, $name = '@laforat Newsletter', array('description' => 'Newsletter widget to add subscription forms on sidebars'), array('width' => '350px'));
    }

    static function get_widget_form( array $instance=array() ) {
        $options_profile = get_option('newsletter_profile');
        $placeholder =( isset( $instance['placeholder'] ) ) ? $instance['placeholder'] : __('Email','laforat');
        ...
        $form .= '<p class="email-letter"><input class="newsletter-email" type="email" placeholder="'. esc_attr( $placeholder ) .'" required name="ne" value="' . esc_attr( $email ) . '" onclick="if (this.defaultValue==this.value) this.value=\'\'" onblur="if (this.value==\'\') this.value=this.defaultValue"/></p>';
        ...
        if (strpos($options_profile['subscribe'], 'http://') !== false) {
            ...
            $form .= '<p class="submit-letter"><input class="newsletter-submit" type="submit" value=""/></p>';
        }
        // hide popup
        $form .= '<div class="tb-newsletter-checkbox"><input type="checkbox" name="hide_popup" id="tb-hide-popup"><span>' . esc_html__("Don’t show this popup again","laforat") .'</span></div>';
    ...
    function widget($args, $instance) {
        ...
        if( $exist_img || $exist_desc ){
            $url = esc_attr($instance['image_url']);
            $buffer .= '<div class="text-left">';
            if( $exist_img ){
                $desc .= "<div class='col-sm-5'><div class='newsletter-bg'><img src='{$url}' alt='background newsletter' class='img-responsive'></div></div>";
            }
            if( $exist_desc ){
                $desc .= '<div><div class="newsletter-desc">';
                $desc .= $before_title . $after_title;
        ...
        if (stripos($instance['text'], '<form') === false) {
            ...
            $form .= laforat_NewsletterWidget::get_widget_form( $instance );
            $form .= '</div>';
            ...
            $buffer = str_ireplace('</form>', '<input type="hidden" name="nr" value="widget"/><div class="tb-newsletter-checkbox"><input type="checkbox" name="hide_popup" id="tb-hide-popup"><span>' . esc_html__("Don’t show this popup again","laforat") .'</span></div></form>', $buffer);
        }
    ...
    function form($instance) {
        ...
        $instance = array_merge(array('title'=>'', 'text'=>'','placeholder'=>'','image_url'=>'','desc'=>''), $instance);
        ?>
        <p>
            <label for="<?php echo $this->get_field_id('title'); ?>">
                <?php esc_html_e('Title','laforat');?>:
                <input class="widefat" id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name('title'); ?>" type="text" value="<?php echo esc_attr($instance['title']); ?>" />
            </label>

            <label for="<?php echo $this->get_field_id('desc'); ?>">
                <?php esc_html_e('Description','laforat');?>:
            ...
            <label for="<?php echo $this->get_field_id('image_url'); ?>">
                <?php esc_html_e('Image on the left side','laforat');?>:
                <input class="widefat" id="<?php echo $this->get_field_id('image_url'); ?>" name="<?php echo $this->get_field_name('image_url'); ?>" type="text" value="<?php echo esc_attr($instance['image_url']); ?>" />
            </label>

            <label for="<?php echo $this->get_field_id('placeholder'); ?>">
                <?php esc_html_e('Placeholder','laforat');?>:
            ...
            <label for="<?php echo $this->get_field_id('text'); ?>">
                <?php esc_html_e('Introduction','laforat');?>:
...
add_action('widgets_init', create_function('', 'return register_widget("laforat_NewsletterWidget");'));
?>
