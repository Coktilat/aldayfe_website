...
<form class="cart" method="post" enctype='multipart/form-data'>
			...
			<?php
				foreach ( $grouped_products as $product_id ) :
					...
					<tr>
						...
						<td class="label">
							<label for="product-<?php echo intval($product_id); ?>">
								<?php echo wp_kses_post($product->is_visible()) ? '<a href="' . get_permalink() . '">' . get_the_title() . '</a>' : get_the_title(); ?>
							</label>
						</td>
	...
	<?php if ( $quantites_required ) : ?>
		...
		<button type="submit" class="single_add_to_cart_button ro-btn-1 alt"><?php echo wp_kses_post($product->single_add_to_cart_text()); ?></button>
