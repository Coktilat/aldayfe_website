			...
			<div class="ro-product-price-meta">
				...
				<?php 
					if ( $product->is_in_stock() ) {
						echo '<div class="stock" >' . __(' Avaiability: ', 'laforat' ) . $product->get_stock_quantity() . '<span>' . __( ' in stock', 'laforat' ) . '</span></div>';
						} else {
						echo '<div class="out-of-stock" >' . __( 'out of stock', 'laforat' ) . '</div>';
					}
				?>
			</div>
			<div class="ro-product-content">
				<?php if($jws_theme_options['jws_theme_single_show_excerpt']) do_action( 'woocommerce_template_single_excerpt' ); ?>
			</div>
