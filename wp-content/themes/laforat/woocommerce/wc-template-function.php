...
function woocommerce_upsell_display( $posts_per_page = 4, $columns = 4, $orderby = 'rand' ) {
	global $tb_options;
	$fullwidth = ( (isset( $_GET['layout']) && $_GET['layout'] ==='fullwidth' ) || ($tb_options['jws_theme_single_sidebar_pos_shop'] ==='jws_theme-sidebar-hidden' ) );
...
if ( ! function_exists( 'jws_theme_woocommerce_sharing' ) ) {
	...
	function jws_theme_woocommerce_sharing() {
		$content = '<!-- Go to www.addthis.com/dashboard to customize your tools -->
			<div class="show_share"><span class="share_all">SHARE </span><ul>
			<li> <a href="http://www.facebook.com/sharer.php?u=https://simplesharebuttons.com" target="_blank">
			<i class="fa fa-facebook"></i>
			</a></li>
			<li>  <a href="https://twitter.com/share?url=https://simplesharebuttons.com&amp;text=Simple%20Share%20Buttons&amp;hashtags=simplesharebuttons" target="_blank">
			<i class="fa fa-twitter"></i>
			</a></li>
			<li>   <a href="https://plus.google.com/share?url=https://simplesharebuttons.com" target="_blank">
			<i class="fa fa-google-plus"></i>
			</a></li>
			<li><a href="http://pinterest.com/pin/create/button/?url={URI-encoded URL of the page to pin}&media={URI-encoded URL of the image to pin}&description={optional URI-encoded description}" class="pin-it-button" count-layout="horizontal">
...
function jws_theme_woo_rename_tabs( $tabs ) {
	...
	$tabs['tag']['title'] = __( 'Tags', 'laforat' );
...
function jws_theme_taxonomy_add_new_meta_field() {
  ...
  <div class="form-field">
    <label for="term_meta[jws_theme_full_width]"><?php _e( 'Show as full width', 'laforat' ); ?></label>
...
function jws_theme_taxonomy_edit_meta_field($term) {
  ...
  ?>
  <tr class="form-field">
  <th scope="row" valign="top"><label for="term_meta[jws_theme_full_width]"><?php _e( 'Full width', 'laforat' ); ?></label></th>
    <td>
      <select name="term_meta[jws_theme_full_width]" id="term_meta[jws_theme_full_width]">
		<option <?php selected( $jws_theme_full_wid, 0 );?> value="0"><?php esc_html_e('No','laforat');?></option>
	   <option <?php selected( $jws_theme_full_wid, 1 );?> value="1"><?php esc_html_e('Yes','laforat');?></option>
		</select>
    </td>
  </tr>
<?php
...
function jws_theme_variable_single_add_to_cart_text( $text ){
	...
	if( $product->is_type( 'variable' ) ){
		$text = __('Select options', 'laforat');
	}
	return $text;
}
...
function jws_theme_product_featured_video(){
	...
	if( isset( $jws_theme_options['jws_theme_video_tab'] ) && $jws_theme_options['jws_theme_video_tab']=='on_thumbnail' ){
		...
		if( $video ){
			echo '<a class="product-video-popup"  data-toggle="tooltip" data-placement="top" title="'. __("View video","laforat") .'" href="'. esc_url($video) .'"><span class="fa fa-play"></span>'. __("Play Video","laforat") .'</a>';
		}
	}
}
...
function jws_theme_woocommerce_sortby_page( $options ){
	global $jws_theme_options;
	$no_page = (int)$jws_theme_options['jws_theme_archive_shop_per_page'];
	if( isset( $jws_theme_options['jws_theme_archive_shop_per_page']) && $no_page = (int)$jws_theme_options['jws_theme_archive_shop_per_page'] && !isset( $options[ $no_page ])){
