			...
			<?php
				...
				if( $exist_thumb ){
					...
					<a href="<?php the_permalink(); ?>" class="tb-thumb-effect" data-tb-thumb="true">
						<?php echo woocommerce_get_product_thumbnail('shop_catalog_large_size'); ?>
						<span>
							<img src="<?php echo esc_url( wp_get_attachment_image_src( array_shift( $att_ids ),'shop_catalog_large_size' )[0] );?>" alt="<?php the_title();?>">
						</span>
								...
								<?php
									...
									if( $show_whishlist ):
										if( function_exists('YITH_WCWL') ):
											$wishlist_text = YITH_WCWL()->is_product_in_wishlist( get_the_ID() ) ? __(' Browse Wishlist','laforat') : __('Add To Wishlist', 'laforat');
								 ?>
