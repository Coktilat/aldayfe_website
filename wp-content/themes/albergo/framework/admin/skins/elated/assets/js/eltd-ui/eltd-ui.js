(function($){
    ...
    eltdUIAdmin.eltdInitDatePicker = eltdInitDatePicker;
    eltdUIAdmin.eltdSelect2 = eltdSelect2;
    eltdUIAdmin.eltdInitSwitch = eltdInitSwitch;
    eltdUIAdmin.eltdInitMediaUploader = eltdInitMediaUploader;
    eltdUIAdmin.eltdInitColorpicker = eltdInitColorpicker;
			...
			frame: function() {
				...
				this._frame.on('update', function() {
					...
					jQuery.ajax({
						type: "post",
						url: ajaxurl,
						data: "action=albergo_elated_gallery_upload_get_images&ids=" + ids,
    ...
    */
	function eltdInitAjaxForm() {
		...
		$(document).delegate(".eltd_ajax_form", "submit", function (a) {
			var b = $(this);
			var c = {
				action: "albergo_elated_save_options"
			};
			jQuery.ajax({
				url: ajaxurl,
				cache: !1,
	...
	function eltdInitDatePicker() {
		$( ".eltd-input.datepicker" ).datepicker( { dateFormat: "yy-mm-dd" });
	}
			...
			eltdImportBtn.on('click', function(e) {
				...
				if (confirm(confirmMessage)) {
					...
					if(import_opt == 'content'){
						for( var i=1; i < 10; i++ ){
							var str;
							if (i < 10) str = 'albergo_content_0'+i+'.xml';
							else str = 'albergo_content_'+i+'.xml';
							jQuery.ajax({
								type: 'POST',
								url: ajaxurl,
								data: {
