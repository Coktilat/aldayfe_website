...
namespace Albergo\Modules\Header\Lib;
/**
 * Class AlbergoElatedHeaderConnector
 *
...
class AlbergoElatedHeaderConnector {
	...
	public function connect( $config = array() ) {
		do_action( 'albergo_elated_pre_header_connect' );
		...
		if ( ! empty( $config['affect_content'] ) ) {
			add_filter( 'albergo_elated_content_elem_style_attr', array( $this, 'contentMarginFilter' ) );
		}
		
		if ( ! empty( $config['affect_title'] ) ) {
			add_filter( 'albergo_elated_title_content_padding', array( $this, 'titlePaddingFilter' ) );
		}
		
		do_action( 'albergo_elated_after_header_connect' );
	}
	 ...
	 *
	 * Hooks to albergo_elated_title_content_padding filter
	 *
	 * @return int
	 */
