...
class ProductInfo implements Lib\ShortcodeInterface {
	...
	public function vcMap() {
		if(function_exists('vc_map')) {
			vc_map(
				array(
					'name'                      => esc_html__( 'Albergo Product Info', 'albergo' ),
					'base'                      => $this->getBase(),
					...
					'params'                    => array(
						array(
							...
							'heading'     => esc_html__( 'Selected Product', 'albergo' ),
							'settings'    => array(
								'sortable'      => true,
								'unique_values' => true
							),
							'admin_label' => true,
							'save_always' => true,
							'description' => esc_html__( 'If you left this field empty then product ID will be of the current page', 'albergo' )
						),
						array(
							'type'        => 'dropdown',
							'param_name'  => 'product_info_type',
							'heading'     => esc_html__( 'Product Info Type', 'albergo' ),
							'value'       => array(
								esc_html__( 'Title', 'albergo' )          => 'title',
								esc_html__( 'Featured Image', 'albergo' ) => 'featured_image',
								esc_html__( 'Category', 'albergo' )       => 'category',
								esc_html__( 'Excerpt', 'albergo' )        => 'excerpt',
								esc_html__( 'Price', 'albergo' )          => 'price',
								esc_html__( 'Rating', 'albergo' )         => 'rating',
								esc_html__( 'Add To Cart', 'albergo' )    => 'add_to_cart',
								esc_html__( 'Tag', 'albergo' )            => 'tag',
								esc_html__( 'Author', 'albergo' )         => 'author',
								esc_html__( 'Date', 'albergo' )           => 'date',
							),
							'admin_label' => true
						),
						array(
							...
							'heading'    => esc_html__( 'Product Info Color', 'albergo' ),
						...
						array(
							...
							'description' => esc_html__( 'Set title tag for product title element', 'albergo' ),
						...
						array(
							...
							'heading'    => esc_html__( 'Featured Image Proportions', 'albergo' ),
							'value'      => array(
						...
						array(
							...
							'heading'     => esc_html__( 'Category Tag', 'albergo' ),
							'value'       => array_flip( albergo_elated_get_title_tag( true ) ),
							'description' => esc_html__( 'Set category tag for product category element', 'albergo' ),
						...
						array(
							...
							'heading'    => esc_html__( 'Add To Cart Skin', 'albergo' ),
							'value'      => array(
								esc_html__( 'Default', 'albergo' ) => '',
								esc_html__( 'White', 'albergo' )   => 'white',
						...
						array(
							...
							'heading'    => esc_html__( 'Add To Cart Size', 'albergo' ),
							'value'      => array(
								esc_html__( 'Default', 'albergo' ) => '',
							...
							'dependency' => array( 'element' => 'product_info_type', 'value'   => array( 'albergo' ) ),
							'group'      => esc_html__( 'Product Info Style', 'albergo' )
						)
					)
				)
	...
	public function getItemTitleHtml( $params ) {
		...
		if ( ! empty( $title ) ) {
			$html = '<' . esc_attr( $title_tag ) . ' itemprop="name" class="eltd-pi-title entry-title" ' . albergo_elated_get_inline_style( $product_info_style ) . '>';
			...
			$html .= '<div class="eltd-pi-category">';
				foreach ( $categories as $cat ) {
					if ( ! empty( $params['category_tag'] ) ) {
						$html .= '<' . esc_attr( $params['category_tag'] ) . ' ' . albergo_elated_get_inline_style( $product_info_style ) . '>';
						...
						$html .= '<a itemprop="url" class="eltd-pi-category-item" href="' . esc_url( get_term_link( $cat->term_id ) ) . '" ' . albergo_elated_get_inline_style( $product_info_style ) . '>' . esc_html( $cat->name ) . '</a>';
					}
				}
			$html .= '</div>';
		}
	...
	public function getItemExcerptHtml( $params ) {
		...
		if ( ! empty( $excerpt ) ) {
			$html = '<div class="eltd-pi-excerpt"><p itemprop="description" class="eltd-pi-excerpt-item" ' . albergo_elated_get_inline_style( $product_info_style ) . '>' . esc_html( $excerpt ) . '</p></div>';
		}
	...
	public function getItemPriceHtml( $params ) {
		...
		if ( $price_html = $product->get_price_html() ) {
			$html = '<div class="eltd-pi-price" ' . albergo_elated_get_inline_style( $product_info_style ) . '>' . $price_html . '</div>';
		}
	...
	public function getItemRatingHtml( $params ) {
		...
		if ( get_option( 'woocommerce_enable_review_rating' ) !== 'no' ) {
			...
			$html = '<div class="eltd-pi-rating" title="' . sprintf( esc_html__( "Rated %s out of 5", "albergo" ), $average ) . '" ' . albergo_elated_get_inline_style( $product_info_style ) . '><span style="width:' . ( ( $average / 5 ) * 100 ) . '%"></span></div>';
		}
	...
	public function getItemAddToCartHtml( $params ) {
		...
		$html .= apply_filters( 'albergo_elated_product_info_add_to_cart_link',
	...
	public function getItemTagHtml( $params ) {
		...
		if ( ! empty( $tags ) ) {
			$html = '<div class="eltd-pi-tag">';
				foreach ( $tags as $tag ) {
					$html .= '<a itemprop="url" class="eltd-pi-tag-item" href="' . esc_url( get_term_link( $tag->term_id ) ) . '" ' . albergo_elated_get_inline_style( $product_info_style ) . '>' . esc_html( $tag->name ) . '</a>';
				}
			$html .= '</div>';
		}
	...
	public function getItemAuthorHtml( $params ) {
		...
		if ( ! empty( $author ) ) {
			$html = '<div class="eltd-pi-author" ' . albergo_elated_get_inline_style( $product_info_style ) . '>' . esc_html( $author ) . '</div>';
		}
	...
	public function getItemDateHtml( $params ) {
		...
		if ( ! empty( $date ) ) {
			$html = '<div class="eltd-pi-date" ' . albergo_elated_get_inline_style( $product_info_style ) . '>' . esc_html( $date ) . '</div>';
		}
	...
	public function productIdAutocompleteSuggester( $query ) {
		...
		if ( is_array( $post_meta_infos ) && ! empty( $post_meta_infos ) ) {
			foreach ( $post_meta_infos as $value ) {
				$data = array();
				$data['value'] = $value['id'];
				$data['label'] = esc_html__( 'Id', 'albergo' ) . ': ' . $value['id'] . ( ( strlen( $value['title'] ) > 0 ) ? ' - ' . esc_html__( 'Title', 'albergo' ) . ': ' . $value['title'] : '' );
				$results[] = $data;
			}
		}
		...
		if ( ! empty( $query ) ) {
			...
			if ( ! is_wp_error( $product ) ) {
				...
				if ( ! empty( $product_title ) ) {
					$product_title_display = ' - ' . esc_html__( 'Title', 'albergo' ) . ': ' . $product_title;
				}
				
				$product_id_display = esc_html__( 'Id', 'albergo' ) . ': ' . $product_id;
