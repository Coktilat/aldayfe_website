<?php
	$author_info_box = esc_attr(albergo_elated_options()->getOptionValue('blog_author_info'));
	$author_info_email = esc_attr(albergo_elated_options()->getOptionValue('blog_author_info_email'));
	$author_id = esc_attr(get_the_author_meta('ID'));
	$social_networks   = albergo_elated_core_plugin_installed() ? albergo_elated_get_user_custom_fields() : false;
    $display_author_social = albergo_elated_options()->getOptionValue('blog_single_author_social') === 'no' ? false : true;
?>
			...
			<div class="eltd-author-description-content">
				<div class="eltd-author-description-image">
					<a itemprop="url" href="<?php echo esc_url(get_author_posts_url($author_id)); ?>" title="<?php the_title_attribute(); ?>" target="_self">
						<?php echo albergo_elated_kses_img(get_avatar(get_the_author_meta( 'ID' ), 120)); ?>
					</a>
				</div>
					...
					<?php if(get_the_author_meta('description') != "") { ?>
						<div class="eltd-author-text">
							<p itemprop="description">
								<?php echo wp_kses(get_the_author_meta('description'), array()); ?>
							</p>
						</div>
					<?php } ?>
							...
							<div class="eltd-author-social-icons clearfix">
								<?php foreach($social_networks as $network){ ?>
									<a itemprop="url" href="<?php echo esc_attr($network['link'])?>" target="_blank">
										<?php echo albergo_elated_icon_collections()->renderIcon($network['class'], 'font_elegant'); ?>
									</a>
								<?php }?>
							</div>
						<?php } ?>
