...
/*
   Class: AlbergoElatedTaxonomyField
   A class that initializes AlbergoElated Taxonomy Field
*/

class AlbergoElatedTaxonomyField implements iAlbergoElatedRender {
	private $type;
	private $name;
	private $label;
	private $description;
	...
	function __construct( $type, $name, $label = "", $description = "", $options = array(), $args = array(), $hidden_property = "", $hidden_values = array() ) {
		...
		add_filter( 'albergo_elated_taxonomy_fields', array( $this, 'addFieldForEditSave' ) );
	}
	...
	public function addFieldForEditSave( $names ) {
		...
		if ( $this->type == 'icon' ) {
			$icons_collections = \AlbergoElatedIconCollections::get_instance()->getIconCollectionsKeys();
			
			foreach ( $icons_collections as $icons_collection ) {
				$icons_param = \AlbergoElatedIconCollections::get_instance()->getIconCollectionParamNameByKey( $icons_collection );
...
abstract class AlbergoElatedTaxonomyFieldType {
	abstract public function render( $name, $label = "", $description = "", $options = array(), $args = array() );
}

class AlbergoElatedTaxonomyFieldText extends AlbergoElatedTaxonomyFieldType {
...
class AlbergoElatedTaxonomyFieldImage extends AlbergoElatedTaxonomyFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $hidden = false) {
		if ( ! isset( $_GET['tag_ID'] ) ) { ?>
			<div class="form-field">
				<label for="<?php echo esc_html( $name ); ?>"><?php echo esc_html( $label ); ?></label>
				...
				<p>
					<input type="button" class="button button-secondary eltd-tax-media-add" name="eltd-tax-media-add" value="<?php esc_html_e( 'Add Image', 'albergo' ); ?>"/>
					<input type="button" class="button button-secondary eltd-tax-media-remove" name="eltd-tax-media-remove" value="<?php esc_html_e( 'Remove Image', 'albergo' ); ?>"/>
				</p>
			</div>
			<?php
		} else {
			...
			<tr class="form-field" <?php if ($hidden) { ?> style="display: none"<?php } ?>>
				...
				<td>
					...
					<p>
						<input type="button" class="button button-secondary eltd-tax-media-add" name="eltd-tax-media-add" value="<?php esc_html_e( 'Add Image', 'albergo' ); ?>"/>
						<input data-termid="<?php echo esc_html( $_GET['tag_ID'] ); ?>" data-taxonomy="<?php echo esc_html( $taxonomy ); ?>" type="button" class="button button-secondary eltd-tax-media-remove" name="eltd-tax-media-remove" value="<?php esc_html_e( 'Remove Image', 'albergo' ); ?>"/>
					</p>
				</td>
			</tr>
			<?php
...
class AlbergoElatedTaxonomyFieldSelect extends AlbergoElatedTaxonomyFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $hidden = false ) {
		...
		if ( ! isset( $_GET['tag_ID'] ) ) { ?>
			<div class="form-field">
				...
				<select
                        class="<?php echo esc_attr($select2)?> form-control eltd-form-element<?php if ($dependence) { echo " dependence"; } ?>"
			...
			<tr class="form-field" <?php if ($hidden) { ?> style="display: none"<?php } ?>>
				...
				<td>
					<select name="<?php echo esc_attr( $name ); ?>"
                            class="<?php echo esc_attr($select2)?> eltd-form-element<?php if ($dependence) { echo " dependence"; } ?>"
						<?php foreach($show as $key=>$value) { ?>
                            data-show-<?php echo str_replace(' ', '',$key); ?>="<?php echo esc_attr($value); ?>"
						<?php } ?>
						<?php foreach($hide as $key=>$value) { ?>
...
class AlbergoElatedTaxonomyFieldIcon extends AlbergoElatedTaxonomyFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $hidden = false ) {
		$options           = \AlbergoElatedIconCollections::get_instance()->getIconCollectionsEmpty();
		$icons_collections = \AlbergoElatedIconCollections::get_instance()->getIconCollectionsKeys();
		...
		if ( ! isset( $_GET['tag_ID'] ) ) { ?>
			...
			<?php foreach ( $icons_collections as $icons_collection ) {
				...
				<div class="form-field eltd-icon-collection-holder" style="display: none" data-icon-collection="<?php echo esc_attr( $icons_collection ); ?>">
					<label for="<?php echo esc_attr( $name ) . '_icon'; ?>"><?php esc_html_e( 'Icon', 'albergo' ); ?></label>
					<select name="<?php echo esc_attr( $name . '_' . $icons_param ) ?>" id="<?php echo esc_attr( $name . '_' . $icons_param ) ?>">
						<?php
						$icons = \AlbergoElatedIconCollections::get_instance()->getIconCollection( $icons_collection );
			...
			<?php foreach ( $icons_collections as $icons_collection ) {
				...
				<tr class="form-field eltd-icon-collection-holder" style="<?php echo esc_attr( $style ); ?>" data-icon-collection="<?php echo esc_attr( $icons_collection ); ?>">
					<th scope="row"><?php esc_html_e( 'Icon', 'albergo' ); ?></th>
					<td>
						<select name="<?php echo esc_attr( $name . '_' . $icons_param ) ?>" id="<?php echo esc_attr( $name . '_' . $icons_param ) ?>">
							<?php
							$icons      = \AlbergoElatedIconCollections::get_instance()->getIconCollection( $icons_collection );
...
class AlbergoElatedTaxonomyFieldColor extends AlbergoElatedTaxonomyFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $hidden = false ) {
...
class AlbergoElatedTaxonomyFieldFactory {
	public function render( $field_type, $name, $label = "", $description = "", $options = array(), $args = array(), $hidden = false ) {
		
		switch ( strtolower( $field_type ) ) {
			case 'text':
				$field = new AlbergoElatedTaxonomyFieldText();
				$field->render( $name, $label, $description, $options, $args, $hidden );
				break;
			
			case 'image':
				$field = new AlbergoElatedTaxonomyFieldImage();
				$field->render( $name, $label, $description, $options, $args, $hidden );
				break;
			
			case 'selectblank':
				$field = new AlbergoElatedTaxonomyFieldSelect();
				$field->render( $name, $label, $description, $options, $args, $hidden );
				break;
			
			case 'icon':
				$field = new AlbergoElatedTaxonomyFieldIcon();
				$field->render( $name, $label, $description, $options, $args, $hidden );
				break;
			
			case 'color':
				$field = new AlbergoElatedTaxonomyFieldColor();
				$field->render( $name, $label, $description, $options, $args, $hidden );
				break;
