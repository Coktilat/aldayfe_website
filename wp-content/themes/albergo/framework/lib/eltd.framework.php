...
/*
   Class: AlbergoElatedFramework
   A class that initializes Albergo Framework
*/
class AlbergoElatedFramework {
	...
	private function __construct() {
		$this->eltdOptions         = AlbergoElatedOptions::get_instance();
		$this->eltdMetaBoxes       = AlbergoElatedMetaBoxes::get_instance();
		$this->eltdTaxonomyOptions = AlbergoElatedTaxonomyOptions::get_instance();
		$this->eltdUserOptions     = AlbergoElatedUserOptions::get_instance();
	}
	...
	public function setSkin( AlbergoElatedSkinAbstract $skinObject ) {
		$this->skin = $skinObject;
	}
}

/**
 * Class AlbergoElatedSkinManager
 *
...
class AlbergoElatedSkinManager {
	...
	/**
	 * @see AlbergoElatedSkinManager::setSkin()
	 */
	...
	private function setSkin( $skinName = 'elated' ) {
		if ( $skinName !== '' ) {
			if ( file_exists( get_template_directory() . '/framework/admin/skins/' . $skinName . '/skin.php' ) ) {
				...
				$skinName = ucfirst( $skinName ) . esc_html__( 'Skin', 'albergo' );
...
/**
 * Class AlbergoElatedSkinAbstract
 *
 * Abstract class that each skin class must extend
 */
abstract class AlbergoElatedSkinAbstract {
	/**
	 * @var string
	 */
	protected $skinName;
	...
	public function loadTemplatePart( $template, $params = array() ) {
		...
		if ( $template !== '' ) {
			include( albergo_elated_find_template_path( 'framework/admin/skins/' . $this->skinName . '/templates/' . $template . '.php' ) );
		}
	}
	...
	public function setShortcodeJSParams() { ?>
		<script>
			window.eltdSCIcon = '<?php echo albergo_elated_get_skin_uri() . '/assets/img/admin-logo-icon.png'; ?>';
			window.eltdSCLabel = '<?php echo esc_html( ucfirst( $this->skinName ) ); ?> <?php esc_html_e( 'Shortcodes', 'albergo' ) ?>';
		</script>
	<?php }
...
/*
   Class: AlbergoElatedOptions
   A class that initializes Albergo Options
*/
class AlbergoElatedOptions {
	private static $instance;
	public $adminPages;
	...
	public function getOptionValue( $key ) {
		...
		if ( array_key_exists( $key, $albergo_options ) ) {
			return $albergo_options[ $key ];
...
/*
   Class: AlbergoElatedAdminPage
   A class that initializes Albergo Admin Page
*/
class AlbergoElatedAdminPage implements iAlbergoElatedLayoutNode {
	public $layout;
	private $factory;
	public $slug;
	public $title;
	...
	function __construct( $slug = "", $title = "", $icon = "" ) {
		$this->layout  = array();
		$this->factory = new AlbergoElatedFieldFactory();
		$this->slug    = $slug;
		$this->title   = $title;
		$this->icon    = $icon;
	}
	...
	public function renderChild( iAlbergoElatedRender $child ) {
		$child->render( $this->factory );
	}
}

/*
   Class: AlbergoElatedMetaBoxes
   A class that initializes Albergo Meta Boxes
*/
class AlbergoElatedMetaBoxes {
	private static $instance;
	public $metaBoxes;
...
/*
   Class: AlbergoElatedMetaBox
   A class that initializes Albergo Meta Box
*/
class AlbergoElatedMetaBox implements iAlbergoElatedLayoutNode {
	public $layout;
	private $factory;
	public $scope;
	public $title;
	...
	public function setTitle( $label ) {
		...
		return $albergo_Framework->getSkin()->getSkinLabel() . ' ' . $label;
	}
}

/*
   Class: AlbergoElatedTaxonomyOptions
   A class that initializes AlbergoElated Taxonomy Options
*/
class AlbergoElatedTaxonomyOptions {
	private static $instance;
...
/*
   Class: AlbergoElatedTaxonomyOption
   A class that initializes AlbergoElated Taxonomy Option
*/
class AlbergoElatedTaxonomyOption implements iAlbergoElatedLayoutNode {
	public $layout;
	private $factory;
	public $scope;
	...
	function __construct( $scope = "" ) {
		$this->layout  = array();
		$this->factory = new AlbergoElatedTaxonomyFieldFactory();
		$this->scope   = $scope;
	}
...
/*
   Class: AlbergoElatedUserOptions
   A class that initializes AlbergoElated User Options
*/
class AlbergoElatedUserOptions {
	private static $instance;
	public $userOptions;
...
/*
   Class: AlbergoElatedUserOption
   A class that initializes AlbergoElated User Option
*/
class AlbergoElatedUserOption implements iAlbergoElatedLayoutNode {
	public $layout;
	private $factory;
	public $scope;
	...
	function __construct( $scope = "" ) {
		$this->layout  = array();
		$this->factory = new AlbergoElatedUserFieldFactory();
		$this->scope   = $scope;
	}
...
if ( ! function_exists( 'albergo_elated_init_framework_variable' ) ) {
	function albergo_elated_init_framework_variable() {
		global $albergo_Framework;
		
		$albergo_Framework = AlbergoElatedFramework::get_instance();
		$eltdSkinManager                 = new AlbergoElatedSkinManager();
		$albergo_Framework->setSkin( $eltdSkinManager->getSkin() );
	}
	
	add_action( 'albergo_elated_before_options_map', 'albergo_elated_init_framework_variable' );
}
?>