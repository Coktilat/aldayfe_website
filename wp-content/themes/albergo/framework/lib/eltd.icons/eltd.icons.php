...
/*
  Class: AlbergoElatedIconCollections
  A class that initializes Albergo Icon Collections
 */
...
class AlbergoElatedIconCollections
{
    ...
    private function initIconCollections() {
        $this->addIconCollection('font_awesome', new AlbergoElatedIconsFontAwesome("Font Awesome", "fa_icon"));
        $this->addIconCollection('font_elegant', new AlbergoElatedIconsFontElegant("Font Elegant", "fe_icon"));
        $this->addIconCollection('ion_icons', new AlbergoElatedIonIcons("Ion Icons", "ion_icon"));
        $this->addIconCollection('linea_icons', new AlbergoElatedLineaIcons('Linea Icons', 'linea_icon'));
        $this->addIconCollection('linear_icons', new AlbergoElatedLinearIcons('Linear Icons', 'linear_icon'));
        $this->addIconCollection('simple_line_icons', new AlbergoElatedSimpleLineIcons('Simple Line Icons', 'simple_line_icon'));
        $this->addIconCollection('dripicons', new AlbergoElatedDripicons('Dripicons', 'dripicon'));
    }
    ...
    public function getIconsMetaBoxOrOption($attributes) {
        ...
        $options = array(
            'font_awesome'      => esc_html__('Font Awesome', 'albergo'),
            'font_elegant'      => esc_html__('Font Elegant', 'albergo'),
            'ion_icons'         => esc_html__('Ion Icons', 'albergo'),
            'simple_line_icons' => esc_html__('Simple Line Icons', 'albergo')
        );
        if ($scope == 'regular') {
            $options = array(
                'font_awesome'      => esc_html__('Font Awesome', 'albergo'),
                'font_elegant'      => esc_html__('Font Elegant', 'albergo'),
                'ion_icons'         => esc_html__('Ion Icons', 'albergo'),
                'linea_icons'       => esc_html__('Linea Icons', 'albergo'),
                'linear_icons'      => esc_html__('Linear Icons', 'albergo'),
                'simple_line_icons' => esc_html__('Simple Line Icons', 'albergo'),
                'dripicons'         => esc_html__('Dripicons', 'albergo')
            );
        }
        ...
        foreach ($icon_collections as $collection_key => $collection_object) {
            ...
            $eltd_icon_pack_container = albergo_elated_add_admin_container(
                array(
                    'parent'          => $parent,
    ...
    public function getVCParamsArray($iconPackDependency = array(), $iconCollectionPrefix = "", $emptyIconPack = false) {
        ...
        $iconPackParams = array(
            'type'        => 'dropdown',
            'heading'     => esc_html__('Icon Pack', 'albergo'),
        ...
        if (is_array($this->iconCollections) && count($this->iconCollections)) {
            foreach ($this->iconCollections as $key => $collection) {
                $iconSetParams[] = array(
                    'type'        => 'dropdown',
                    'heading'     => esc_html__('Icon', 'albergo'),
    ...
    public function getIconWidgetParamsArray() {
        ...
        $iconPackParams[] = array(
            'type'    => 'dropdown',
            'title'   => esc_html__('Icon Pack', 'albergo'),
            'name'    => 'icon_pack',
            'options' => array(
                'font_awesome'      => esc_html__('Font Awesome', 'albergo'),
                'font_elegant'      => esc_html__('Font Elegant', 'albergo'),
    ...
    public function getIconCollectionIcons(iAlbergoElatedIconCollection $collection) {
    ...
    public function getIconCollectionsEmpty($no_empty_key = "") {
        $array = array();
        $array[$no_empty_key] = esc_html__( 'No Icon', 'albergo');
        foreach ($this->iconCollections as $key => $collection) {
            $array[$key] = $collection->title;
        }
    ...
    public function getIconCollectionsVCEmpty() {
        $vc_array = array();
	    $vc_array[ esc_html__( 'No Icon', 'albergo' ) ] = '';
        foreach ($this->iconCollections as $key => $collection) {
            $vc_array[$collection->title] = $key;
        }
...
if (!function_exists('albergo_elated_activate_theme_icons')) {
    function albergo_elated_activate_theme_icons() {
        global $albergo_IconCollections;
        $albergo_IconCollections = AlbergoElatedIconCollections::get_instance();
    }

    add_action('after_setup_theme', 'albergo_elated_activate_theme_icons');
}