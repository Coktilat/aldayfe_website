...
//***
            ...
            //fix 3.3.0
            // add_filter('woocommerce_variation_prices_price', array($this, 'woocommerce_variation_prices_fix_3_3'), 9999, 3);
            ...
            // add_filter('woocommerce_variation_prices_regular_price', array($this, 'woocommerce_variation_prices_regular'), 9999, 3);
            // add_filter('woocommerce_variation_prices_sale_price', array($this, 'woocommerce_variation_prices_sale'), 9, 3);
        }
//***
...
//*** checking and assigning data
    ...
    public function plugin_action_links($links) {
        return array_merge(array(
            ...
            '<a target="_blank" href="' . esc_url('https://currency-switcher.com/documentation/') . '">' . __('Documentation', 'woocommerce-currency-switcher') . '</a>'
                ), $links);
    ...
    public function widgets_init() {
        ...
        wp_register_script('wc-price-slider_33', WOOCS_LINK . 'js/price-slider_33.js', array('jquery', 'jquery-ui-slider', 'wc-jquery-ui-touchpunch'));
    }
...
//wp_enqueue_scripts('jquery');
        ...
        if (in_array($typenow, wc_get_order_types('order-meta-boxes'), true)) {
            ...
            <style type="text/css">
                .woocs_recalculate_all_orders_curr_button .woocs_ajax_preload{
                    display:none;
                    vertical-align: middle;
                }
                .tablenav a.woocs_recalculate_all_orders_curr_button {
                    position: relative;
...
//***
            ...
            if ($this->is_fixed_enabled) {
                //$cart_amount=($cart_item['data']->get_price()/$currencies[$this->current_currency]['rate'])*$cart_item['quantity'];
            }
...
//***
    ...
    public function woocs_add_approx_to_price($price_html, $product) {
        ...
        $currencies = $this->get_currencies();
        $info = "";
        if ($user_currency != $this->current_currency) {
            $tmp_curr_currency = $this->current_currency;
            $this->set_currency($user_currency);
...
//***
    ...
    private function get_cart_item_price_html($wc_price) {
        ...
        $html .= apply_filters('woocs_get_approximate_amount_text', sprintf(__('(Approx. %s)', 'woocommerce-currency-switcher'), $wc_price), $wc_price);
        $html .= '</div>';
        return $html;
    }
        ...
        if (isset($_GET['tab']) AND $_GET['tab'] == 'woocs') {
            ...
            <div class="notice notice-warning is-dismissible" id="woocs_alert_notice">
                <p class="plugin-card-woocommerce-currency-switcher">
                    <b style="color:red;">Attention</b>: If WOOCS settings panel looks incorrect: Clear the browser  - <a href="https://currency-switcher.com/reset-page-cache-browser/" target="_blank">read this please.</a>
                </p>
                <hr />
                <p class="plugin-card-woocommerce-currency-switcher">
    ...
    public function prepare_default_currencies() {
        ...
        switch ($wc_currency) {
            ...
            default :
                ...
                $default['USD']['description'] = __('change the rate and this description to the right values', 'woocommerce-currency-switcher');
                break;
        }
